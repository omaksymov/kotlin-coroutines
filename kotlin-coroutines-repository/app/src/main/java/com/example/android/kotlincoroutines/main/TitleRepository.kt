...
suspend fun <T> FakeNetworkCall<T>.await(): T {
    return suspendCoroutine { continuation ->
        addOnResultListener { fakeNetworkResult ->
            when (fakeNetworkResult) {
                is FakeNetworkSuccess<T> -> continuation.resume(fakeNetworkResult.data)
                is FakeNetworkError<T> -> continuation.resumeWithException(fakeNetworkResult.error)
            }
        }
    }
}
